...
const initialize = (state) => {
    ...
    //Wait for first round of heartbeat messages
    ...
    setTimeout(() => {
        ...
        multicaster.getEventEmitter().on(monsterActionEvent, (monsterAction) => {
                ...
                global.log.push('main', 'server uses broadcasted monster actions' + JSON.stringify(monsterAction));
        });
        ...
        //Send Heartbeat messages and remove dead peer playerlists
        setInterval(() => {
            ...
            //If no leader in peerlist
            let leader = heartbeat.getLeader();
            global.log.push('main', 'Leader: ' + JSON.stringify(leader));
            let isLeader = false;
            if (leader === undefined) {
                leader = heartbeat.electLeader();
                const address = multicaster.sender.address();
                //Check if you are the leader
                if ((address.address === anyIp || leader.address === address.address) && leader.port === address.port) {
                    isLeader = true;
                    updateSimulation(simulation, multicaster, monsterAnnotation);
                }
            }

            multicaster.sendMessage(heartbeatEvent, {...heartbeat.heartbeatMessage(Date.now()), isLeader});
        }, heartbeatInterval);
...
const updateSimulation = (simulation, multicaster, monsterAnnotation) => {
    //TODO check if already spawned monsters
    const spawnActions = simulation.spawnAgents(Date.now(), monsterAnnotation);
    spawnActions.forEach((spawnAction) => multicaster.sendMessage(monsterActionEvent, spawnAction));
    ...
    setInterval(() => {
        ...
        global.log.push('main', 'send monster actions as leader');
